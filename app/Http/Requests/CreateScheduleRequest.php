    ...
    public function rules()
    {
        return [
            ...
            'start_time_time' => ['required', 'date_format:H:i', 'before:end_time_time', function ($attribute, $value, $fail) {
                $pattern = '/^(?:\d{4})\-(?:\d{1,2})\-(?:\d{1,2}) (?P<hour>\d{1,2}):(?P<minute>\d{1,2})$/';
                if (!preg_match($pattern, $this->input('start_time_date') . ' ' . $this->input('start_time_time'), $matches)) {
                    // Y/m/d H時i分形式ではない場合はエラー処理
                    $fail('out');
                    return;
                }
                ...
                //dd($startTime,$endTime,$diffInMinutes);
                if ($diffInMinutes <= 5) {
